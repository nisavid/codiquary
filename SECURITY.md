...
secret-bearing provider payloads. Do not test against a live deployment or
production authority without its owner's separate explicit permission.

The maintainer will coordinate disclosure with the reporter. The project may
suspend qualification, warn against an affected revision, or publish an
incompatible current release when authority or verification requires it. An
advisory is not a promise to repair every historical version.

## Security boundary

Repository checks, CI attestations, hosting controls, and maintainer approval
are defense in depth. None substitutes for the release authority, status
authority, independent verification, or protected-consumer admission defined by
